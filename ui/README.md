...
探索パネルで検索・言語の絞り込み・件数を確認します。地図上の語をホバーすると詳細と関係線を一時表示し、クリックで固定します。別の語をホバーして離すと固定した語に戻り、固定した語の再クリックで解除します。[デザインシステム](../DESIGN.md)に視覚と操作の基準を記録しています。
...
The explorer panel holds search, language filters, and counts. Hovering over a map word previews its details and relations; clicking pins them. Hovering over another word temporarily replaces the preview, and leaving restores the pinned word. Clicking the pinned word again clears it. The [design system](../DESIGN.md) defines visual and interaction rules.
